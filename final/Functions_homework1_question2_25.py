...
def generate_unsupervised_rbf_variables(train_X, INPUT_UNITS, OUTPUT_UNITS, HIDDEN_UNITS, RHO_CONSTANT, SIGMA_CONSTANT):
...
def generate_unsupervised_rbf_y_hat_function(X, omega, pi):
    m = []
...
def train_generic_model(sess, optimizer, train_loss, omega, X, y, train_X, train_y,
                        EPOCHS=10000, COMPUTE_TIME=True, tol=1e-5, target="MLP"):
    toc = None
    if COMPUTE_TIME:
        tic = time()
